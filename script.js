/*
  ...
  - Ganti foto di assets/photos/
  - Tambah lagu dengan nama assets/music/music.mp3
*/
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (personName) personName.textContent = CONFIG.personName;
  ...
  function openGift(event) {
    ...
    createBurst();
    if (navigator.vibrate) navigator.vibrate([35, 45, 25]);
    ...
    setTimeout(() => {
      ...
      setTimeout(() => giftScreen.remove(), 1100);
    }, CONFIG.giftDelayToMain);
  }

  // Event dibuat berlapis supaya aman di browser HP biasa, tanpa mode situs desktop.
  ...
  ["touchstart", "click"].forEach((eventName) => {
    giftScreen.addEventListener(eventName, (event) => {
      if (event.target === giftScreen || event.target.classList.contains("tap-hint")) openGift(event);
    }, { passive: false });
  });
  ...
  musicBtn?.addEventListener("click", async () => {
    try {
      ...
      musicBtn.title = "Masukkan file assets/music/music.mp3 dulu.";
      console.warn("Musik belum bisa diputar. Pastikan file assets/music/music.mp3 ada.", error);
    }
  });
  ...
  // Garden interactive messages
  flowers.forEach((flower) => {
    const showMessage = () => {
      flowers.forEach((item) => item.classList.remove("active"));
      flower.classList.add("active");
      if (flowerMessage) {
        flowerMessage.textContent = flower.dataset.message || "Semoga semua hal baik datang kepadamu.";
      }
      createMiniConfetti(flower.getBoundingClientRect().left + flower.offsetWidth / 2, flower.getBoundingClientRect().top + 60, 18);
    };
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape") closeWishModal();
  });
  ...
  function createMiniConfetti(x, y, amount = 40) {
    ...
    for (let i = 0; i < amount; i++) {
      ...
      confetti.style.animationDelay = `${random(0, .16)}s`;
...
function initStars() {
  ...
  function draw() {
    ...
    // Heart constellation subtle in the top area
    const cx = width * 0.5;
    ...
    for (let t = 0; t < Math.PI * 2; t += Math.PI / 13) {
      ...
      points.push({ x: cx + x * 5.2, y: cy + y * 5.2 });
    }
    ...
    points.forEach((p, i) => {
      ctx.beginPath();
      ctx.fillStyle = i % 3 === 0 ? "rgba(255, 217, 138, .38)" : "rgba(255, 255, 255, .18)";
